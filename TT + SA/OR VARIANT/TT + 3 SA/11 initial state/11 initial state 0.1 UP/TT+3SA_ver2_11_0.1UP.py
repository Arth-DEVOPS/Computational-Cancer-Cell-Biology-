...
def update(A, B, C):
    ...
    if randomizer <= 0.33:
        A = (not B) or (not C) or (A)
        return A,B,C

    elif 0.33 < randomizer <= 0.66:
        B = (not A) or (not C) or (B)
        return A,B,C

    else:
        C = (not A) or (not B) or (C)
        return A,B,C
...
def main():
    ...
    with open("TT + 3 SA ver2 MRT " + "p=0.1 UP perturbation " + " output.tsv", "w") as csvfile:
    ...
    with open("TT + 3 SA ver2 Trajectory" + "p=0.1 UP perturbation " + " output.tsv", "w") as csvfile:
        writer = csv.writer(csvfile, dialect="excel-tab")
