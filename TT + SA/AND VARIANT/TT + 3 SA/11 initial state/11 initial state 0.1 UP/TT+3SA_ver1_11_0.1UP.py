...
def update(A, B, C):
    ...
    if randomizer <= 0.33:
        ...
        B = (not A) and (not C) and (B)
        return A,B,C

    else:
        C = (not A) and (not B) and (C)
        return A,B,C
...
def main():
    ...
    with open("TT + 3 SA ver1 MRT " + "p=0.1 UP perturbation " + " output.tsv", "w") as csvfile:
    ...
    with open("TT + 3 SA ver1 Trajectory" + "p=0.1 UP perturbation " + " output.tsv", "w") as csvfile:
        writer = csv.writer(csvfile, dialect="excel-tab")
